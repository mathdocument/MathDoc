...
Database revisions are independent of source-code Git commits. Every graph mutation creates a TerminusDB commit; `mdc history --proj myproject/main` and `mdc branch create --proj myproject/main NAME` expose basic history and branching. Native TerminusDB provides merge/rebase operations; no merge UI is included.
