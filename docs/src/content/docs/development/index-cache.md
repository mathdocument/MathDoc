...
Lean keys include the environment, stable module identity, Lean source and direct dependency keys recursively. Editing text, titles, Rocq or LaTeX leaves these keys unchanged. A Lean edit updates keys only for affected referrers; their unchanged source hashing state is reused. Graph changes recompute graph metadata. Compilation inputs share immutable node and project snapshots. An editor tracks which node versions it has materialized, so selecting unchanged nodes does not reread generated source files.
