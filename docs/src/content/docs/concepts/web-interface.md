...
Lean blocks initially show a local Monaco editor with syntax highlighting, editing and **Save**, without starting a Lean server or reserving a server slot. Click **Start Lean server** (the play icon) to enable checking and native Infoview on the right, including goals, diagnostics and interactive widgets. This button then becomes **Reload environment**. Both modes use the same Lean grammar, font and light/dark themes; unsaved text survives startup and reload.
