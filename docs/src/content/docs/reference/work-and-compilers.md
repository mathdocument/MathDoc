...
mdc work 'Theorem' --build
```

Line and column are zero-based LSP positions (columns use UTF-16 code units). `work` is a compatibility spelling for a Lean check; it no longer exports an editable mirror.
