...
fn lib_source(req: &CompilerReq, srctype: &str) -> Result<(PathBuf, PathBuf)> {
    let lib_root = req.mdcroot.join(".mdc").join(srctype).join("Lib");
    let lib_root = lib_root
        .canonicalize()
        .with_context(|| format!("canonicalizing Lib source root {}", lib_root.display()))?;
    let source = req
        .source
        .canonicalize()
        .with_context(|| format!("canonicalizing compiler source {}", req.source.display()))?;
    let relative = source
...
mod tests {
    ...
    fn test_lean_builds_imports_from_lib_tree() {
        ...
        assert!(result.result, "lean compilation failed: {}", result.stderr);
        assert!(mdcroot
    ...
    fn test_rocq_imports_previous_lib_build_artifacts() {
        ...
        assert!(result.result, "Rocq compilation failed: {}", result.stderr);
        ...
        assert!(!stale_import.result, "stale Rocq import remained available");
