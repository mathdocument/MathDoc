...
use crate::{config::Settings, service::{ApiError, RunningService, Service}};
...
pub(crate) async fn status() -> Result<Value> {
    ...
    for db in crate::store::Terminus::from_env()?.projects().await? {
        ...
        let running = server.as_ref().zip(branch.as_ref())
            .is_some_and(|(s, b)| s.pid == b.pid && s.port == b.port);
        let url = server.as_ref().filter(|_| running)
...
impl Server {
    async fn start_branch(&self, project: String) -> Result<Value> {
        ...
        let (database, name) = crate::config::project_parts(&project)?;
        {
            ...
            anyhow::ensure!(!branches.contains_key(&project), "{project} is already running or changing state");
            branches.insert(project.clone(), None);
        }
        let opened = async {
            let service = Service::open(crate::store::Database::from_env(database.into(), name.into())?).await?;
            let root = service.db.cache_path()?;
            let info = RunningService {
                port: self.info.port, pid: self.info.pid, token: service.token.clone(),
                public_origin: self.info.public_origin.clone(),
            };
            std::fs::write(root.join("service.lock"), serde_json::to_vec(&info)?)?;
            Ok::<_, anyhow::Error>(Branch { router: crate::service::router(service.clone()), service })
        }.await;
        match opened {
            Ok(branch) => { self.branches.lock().await.insert(project.clone(), Some(branch)); }
            Err(error) => { self.branches.lock().await.remove(&project); return Err(error); }
        }
        Ok(json!({"project":project,"port":self.info.port,"pid":self.info.pid,
    ...
    async fn stop_branch(&self, project: String, token: String) -> Result<Value, ApiError> {
        ...
        let branch = {
            let mut branches = self.branches.lock().await;
            let slot = branches.get_mut(&project).ok_or_else(|| stopped(&project))?;
...
pub(crate) async fn serve(port: u16) -> Result<()> {
    ...
    let state = Arc::new(Server {
        info: RunningService { port:listener.local_addr()?.port(),pid:std::process::id(),
            token:uuid::Uuid::new_v4().to_string(), public_origin:settings.public_origin()? },
        branches: Mutex::new(HashMap::new()), operations: RwLock::new(()),
        closing: AtomicBool::new(false), stop: tokio::sync::Notify::new(),
    });
    let router = Router::new()
        .route("/api/status", get(|| async { status().await.map(Json).map_err(ApiError::from) }))
        ...
        .layer(axum::middleware::from_fn_with_state(state.clone(), check_origin));
...
async fn check_origin(State(state): State<Arc<Server>>, request: Request, next: axum::middleware::Next) -> Response {
    if state.closing.load(Ordering::Acquire) { return stopped("server").into_response(); }
    if !crate::service::allowed_origin(request.headers(), state.info.public_origin.as_deref()) {
        return ApiError(StatusCode::FORBIDDEN, "same-origin requests from the configured host required".into()).into_response();
    }
    next.run(request).await
}

fn authorize_token(actual: &str, expected: &str) -> Result<(), ApiError> {
    if actual != expected { return Err(ApiError(StatusCode::FORBIDDEN, "use mdc start/stop to manage services".into())); }
    Ok(())
}
fn token(headers: &HeaderMap) -> &str {
    headers.get("x-mdc-service").and_then(|v| v.to_str().ok()).unwrap_or("")
}
fn stopped(project: &str) -> ApiError {
    ApiError(StatusCode::SERVICE_UNAVAILABLE, format!("{project} is stopped or changing state; run mdc start {project}"))
}
async fn stop(State(state): State<Arc<Server>>, headers: HeaderMap) -> Result<Json<Value>, ApiError> {
    authorize_token(token(&headers), &state.info.token)?;
    state.stop.notify_one();
    Ok(Json(json!({"stopping":true})))
}

async fn project_request(State(state): State<Arc<Server>>, mut request: Request) -> Result<Response, ApiError> {
    let path = request.uri().path().to_owned();
    let Some(rest) = path.strip_prefix("/p/") else {
        if path.starts_with("/api/") { return Err(ApiError(StatusCode::NOT_FOUND, "API endpoint not found".into())); }
        if request.method() != Method::GET { return Err(ApiError(StatusCode::METHOD_NOT_ALLOWED, "GET required".into())); }
        return Ok(crate::web::assets::serve_asset(request.uri().clone()));
    };
    let mut segments = rest.splitn(3, '/');
    let project = format!("{}/{}", segments.next().unwrap_or(""), segments.next().unwrap_or(""));
    ...
    if !suffix.starts_with("api/") {
        if request.method() != Method::GET { return Err(ApiError(StatusCode::METHOD_NOT_ALLOWED, "GET required".into())); }
        return Ok(crate::web::assets::serve_asset(format!("/{suffix}").parse().map_err(anyhow::Error::from)?));
    }
    if matches!(suffix, "api/service/start" | "api/service/stop") {
        if request.method() != Method::POST { return Err(ApiError(StatusCode::METHOD_NOT_ALLOWED, "POST required".into())); }
        ...
        if suffix == "api/service/start" {
            ...
            return tokio::spawn(async move { state.start_branch(project).await }).await
                .map_err(anyhow::Error::from)?.map(Json).map(IntoResponse::into_response).map_err(ApiError::from);
        }
        return tokio::spawn(async move { state.stop_branch(project, token).await }).await
            .map_err(anyhow::Error::from)?.map(Json).map(IntoResponse::into_response);
    }
    let router = state.branches.lock().await.get(&project)
        .and_then(|slot| slot.as_ref()).map(|b| b.router.clone()).ok_or_else(|| stopped(&project))?;
    let route = request.uri().path_and_query().context("missing request path")?.as_str()
        .strip_prefix(&format!("/p/{project}")).context("invalid project path")?;
