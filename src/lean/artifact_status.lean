...
@[noinline] def inspect (path : System.FilePath) : IO (Bool × Array CompactedRegion) := do
  let paths ← if ← path.addExtension "server" |>.pathExists then
    pure #[path, path.addExtension "server", path.addExtension "private"]
  else pure #[path]
...
unsafe def main : IO Unit := do
  ...
  let paths : Array String ← IO.ofExcept (Json.parse input >>= fromJson?)
  let mut results : Array (Option Bool) := #[]
  for path in paths do
    try
      let (hasSorry, regions) ← inspect path
      ...
      (← IO.getStderr).putStrLn s!"Lean artifact status: {path}: {e}"
